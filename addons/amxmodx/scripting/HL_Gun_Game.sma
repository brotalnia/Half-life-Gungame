...
public plugin_init()
{
	...
	register_forward(FM_SetModel, "fSetModel")
...
public load_settings()
{
	...
	if(get_pcvar_num(gg_enabled))
	{
		...
		if(file_exists(cfgpath)) {
			...
			server_exec()
			new spd = get_pcvar_num(gg_runspeed)+200
...
public fragsCmd(id, level, cid)
{
	...
	read_argv(1, arg1, 2)
	read_argv(2, arg2, 2)
	new level = str_to_num(arg1)
	if (0 <= level <= MAXLEVEL-1)
	{
		new fragreq = str_to_num(arg2)
		if (fragreq>0)
		{
			frags[level] = fragreq
		}
...
ResetScore()
{
	...
	for(new i=0; i<inum; ++i)
	{
		...
		set_pdata_int(player, g_iDeathOffset, 0)
		set_pev(player, pev_solid, 0)
...
public eResetHud(id)
{
	...
	set_task(0.1, "handle_reset_hud", id)
}
public handle_reset_hud(id)
{
	// equips players, sets their speed, and shows information about their level
	// called by eResetHud event
	...
	set_user_maxspeed(id, get_pcvar_float(gg_runspeed))
	status_display(id)
}
Equip_Player(id)
{
	// gives players weapons and ammo and switches to new weapon
	// called from handle_reset_hud, giveCmd, level_up and level_down
	
	if(!get_pcvar_num(gg_enabled) || !is_user_alive(id))
		return
	...
	{		
		if (jumbot)
			set_pdata_int(id, 489, 99, 5)
		else if (won)
			set_pdata_int(id, 307, 99, 5)
		else
			set_pdata_int(id, 312, 99, 5)
	}
	else if (level==12)
	{			
		if (jumbot)
			set_pdata_int(id, 495, 99, 5)
		else if (won)
			set_pdata_int(id, 313, 99, 5)
		else
			set_pdata_int(id, 318, 99, 5)
	}
	else if (level==13)
	{
		if (jumbot)
			set_pdata_int(id, 494, 99, 5)
		else if (won)
			set_pdata_int(id, 312, 99, 5)
		else
			set_pdata_int(id, 317, 99, 5)
	}
}
public eDeathMsg()
{
	...
	array[0] = victim
	array[1] = killer
	array[2] = weapon_name[0]
	array[3] = weapon_name[1]
	array[4] = weapon_name[2]
	array[5] = weapon_name[3]
	array[6] = weapon_name[4]
	array[7] = weapon_name[5]
	array[8] = weapon_name[6]
	
	set_task(0.1, "handle_death_event", 0, array, sizeof(array))
}
public handle_death_event(Params[])
{
	// reward or punish kills and deaths
	// called by eDeathMsg event
	
	new victimid = Params[0]
	new killerid = Params[1]
	
	if((victimid != killerid) && is_user_connected(victimid) && (killerid != 0))
	{
		new victimteam[32]
		get_user_team(victimid, victimteam, 31)
		new killerteam[32]
		get_user_team(killerid, killerteam, 31)
		
		if (!((get_pcvar_num(mp_teamplay)==1) && (equal(victimteam, killerteam))))
		{
			new victim_name[32]
			get_user_name(victimid, victim_name, 31)
			new killer_name[32]
			get_user_name(killerid, killer_name, 31)
			
			if (victimid != g_dominating)
			{
				// normal kill
				ColorChat(victimid, killerid, "^^2[GunGame] ^^9: You were killed by ^x03%s^^9 who is at level ^^8%i^^9.", killer_name, g_level[killerid])
				if ((g_level[killerid]<MAXLEVEL-1) || contain(Params,"crowbar")!=-1)
				{
					// prevent skipping of last level
					ColorChat(killerid, victimid, "^^2[GunGame] ^^9: You killed ^x03%s^^9 who is at level ^^8%i^^9.", victim_name, g_level[victimid])
					frag_up(killerid)
				}
			}
			else
			{
				...
				ColorChat(killerid, victimid, "^^2[GunGame] ^^9: You have slain ^x03%s^^9 and earned a level!", victim_name)
			...
			level_down(killerid, 2) // punish team killers
	}
	else if ((victimid==killerid) && (get_pcvar_num(gg_suicide)) && is_user_connected(victimid))
		level_down(victimid, 1) // punish suicide

	// related to domination mechanic
	g_lastdeath[victimid] = g_level[victimid]
	if (victimid == g_dominating)
		g_dominating = 0
}
public fSetModel(ent, const model[])
{
	// checks for weaponboxes and removes them
	// called when a new model is created
	
	if (!get_pcvar_num(gg_enabled) || !pev_valid(ent) || !equali(model, "models/w_weaponbox.mdl"))
...
status_display(id)
{
	...
	// called from handle_reset_hud, giveCmd, check_level, frag_up, level_up and level_down
...
public eCurWeapon(id)
{
	...
	set_task(0.1, "handle_weapon_event", id)
	
	return PLUGIN_HANDLED
}
public handle_weapon_event(id)
{
	// check what weapon the player is using and change it if it is not for his level
	// called by eCurWeapon event
...
FillClipAmmo(id)
{
	...
	if (level<5)
	{
		...
		new weaponEntity = fm_find_ent_by_owner(40, weaponName, id)
		if (weaponEntity!=0)
		{
			new ammoOffset
			if (!jumbot)
			...
			{
				case 0:
					set_pdata_int(weaponEntity, ammoOffset, 17, 4)
				case 1:
					set_pdata_int(weaponEntity, ammoOffset, 6, 4)
				case 2:
					set_pdata_int(weaponEntity, ammoOffset, 8, 4)
				case 3:
					set_pdata_int(weaponEntity, ammoOffset, 50, 4)
				case 4:
					set_pdata_int(weaponEntity, ammoOffset, 5, 4)
			}
		}
	}
}
...
frag_up(id)
{
	...
	// called from handle_death_event
...
level_up(id)
{
	...
	// called from handle_death_event and frag_up
...
make_dominator(id)
{
	...
	lightning_strike(id)
	shake_screen(id)
	set_user_maxspeed(id, get_pcvar_float(gg_runspeed)+200.0)
...
public actionChangemap()
{
	...
	if (is_plugin_loaded("Nextmap Chooser")!=-1) {
		...
		{
			new szMaps[32][33], iTextLength1, iLine1, iterations = 0
			while (read_file("mapcycle.txt", iLine1, szMaps[iterations], 32, iTextLength1) != 0)
			{
				iterations++
				iLine1++
				if (iterations>30)
			...
			new maxmaps
			if (iterations<31)
				maxmaps=iterations-1
			else
				maxmaps=31
			
			static nextmape[32]
			get_cvar_string("amx_nextmap", nextmape, 31)
			server_cmd("amx_votemap %s %s %s %s", nextmape, szMaps[random_num(0,maxmaps)], szMaps[random_num(0,maxmaps)], szMaps[random_num(0,maxmaps)])
		}
		else
		{
			static currentmap[32]
			get_mapname(currentmap, 31)
			if (is_plugin_loaded("NextMap")!=-1)
			{
				static nextmape[32]
				get_cvar_string("amx_nextmap", nextmape, 31)
				server_cmd("amx_votemap %s %s", nextmape, currentmap)
			}
			else
				server_cmd("amx_votemap %s %s", currentmap, "crossfire")
		}
	}
	else
...
hideObjects() // BAILOPAN org + tweaks KWo and KRoT@L
{
	...
	for (new i=0; i<S_OBJS; i++) {	
		...
		tEnt = fm_find_ent_by_class(-1, StripObjs[i])
		while (tEnt > 0) {
			tfEnt = fm_find_ent_by_class(tEnt, StripObjs[i])
			if (objectives == MAX_OBJ - 1)  // KWo
				objectives = 0
			else
				objectives++
			if (fm_is_valid_ent(tEnt)) {
				pev(tEnt, pev_origin, ObjVecs[objectives])
...
destroyEntities()
{
	...
	for (new i=0; i<S_ENTS; i++) {	
		tEnt = fm_find_ent_by_class(-1, StripEnts[i])
		while (tEnt > 0) {
			tfEnt = fm_find_ent_by_class(tEnt, StripEnts[i])
			if (fm_is_valid_ent(tEnt))
				fm_remove_entity(tEnt)		// KWo
			tEnt = tfEnt
		}
	}
}
public destroyWeaponbox(ent)
{
	...
	// called from fSetModel when a weaponbox is created
...
restoreObjectives()   // KWo
{
	...
	for (i=1; i<=objectives; i++) {
		if (fm_is_valid_ent(ObjEntsId[i]))
		{
			set_pev(ObjEntsId[i], pev_rendermode, kRenderNormal)
			fm_entity_set_origin(ObjEntsId[i], ObjVecs[i])
		}
	}
	objectives = 0
